...
lr=0.08 # 0.002                                                                #learning rate
lambda1=0.1  # 100 # 3 # .1                                                        #regularisation rate
hidden1 = 400 #hidden layer 1
hidden2 = 200 #hidden layer 2
hidden3=100
...
z3=T.dot(z2, w3) + b3
if acti_type=='sigmoid':
    h3 = 1 / (1 + T.exp(-z3))              # hidden layer 2
elif acti_type=='linear':
    h3 = z3
elif acti_type=='tanh':
    h3=T.tanh(z3)

p_1 = 1 / (1 + T.exp(-T.dot(h3, w4) - b4))               # Probability that target = 1
prediction = p_1 #> 0.5                                   # The prediction thresholded
xent = - y * T.log(p_1) - (1-y) * T.log(1-p_1)             # Cross-entropy loss function
cost = xent.mean() + lambda1 * ((w1 ** 2).sum() +
       (w2 ** 2).sum() + (w3 ** 2).sum() +(w4 ** 2).sum()+
       (b1 ** 2).sum() + (b2 ** 2).sum() + 
       (b3 ** 2).sum()+ (b4 ** 2)) #/ \
       #(xdim * hidden1 + hidden1 * hidden2 + hidden2 + hidden1 + hidden2 + 1) # The cost to minimize
gw4,gb4,gw3, gb3, gw2, gb2, gw1, gb1, gx = T.grad(cost, [w4,b4,w3, b3, w2, b2, w1, b1, x])        # Compute the gradient of the cost


# Compile
train = theano.function(
          inputs=[x,y],
          outputs=[gx, w1, w2, w3,w4],updates=(
